...
int bfs(struct koordinat awal, int bykLantai, int bykRow, int bykCol){
	...
	while(qHead != NULL){
		...
		for(int i=0; i<6; i++){
			...
			if(nextLnt >= 0 && nextRow >= 0 && nextCol >= 0
				 && nextLnt < bykLantai && nextRow < bykRow && nextCol < bykCol
				 && !visited[nextLnt][nextRow][nextCol]){
...
int main(){
	...
	while(1){
		scanf("%d %d %d", &bykLantai, &bykRow, &bykCol);
		getchar();
		...
		int hasil = bfs(awal, bykLantai, bykRow, bykCol) + 1;
